...
APP_TITLE = "Cracko298's 3DS Shader Assembler/Editor - DVLB/DVLP/DVLE"
...
class EmbeddedPicassoAssembler:
    ...
    def _pragma_output_map(self, args: List[str]) -> None:
        ...
        mask = parse_pica_dest_mask(mask_s)
        reg_id = int(reg[1:], 0)
    ...
    def _conditional_instruction(self, mnemonic: str, expanded: str) -> None:
        ...
        if mnemonic == "BREAKC":
            ...
            return
        if mnemonic == "IFC":
            raise PicassoCompileError("ifc block directive assembly is not implemented yet; use raw/expanded flow-control form")
        if len(args) != 2:
    ...
    def _normalize_picasso_instruction(self, line: str) -> str:
        ...
        if mnem == "CMP" and len(args) == 4:
            # Picasso manual syntax: cmp src1, opx, opy, src2. Editor syntax: cmp opx, opy, src1, src2.
            if args[1].strip().upper() in {v.upper() for v in CMP_OP_NAMES.values()}:
