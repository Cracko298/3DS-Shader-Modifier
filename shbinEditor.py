...
def parse_arithmetic_asm_line(line: str) -> Tuple[str, Dict[str, Any], Dict[str, Any]]:
    #Parse a small nihstro/picasso-like arithmetic line.
    #Supports: add/dp3/dp4/dph/dst/mul/sge/slt/max/min/mov/mova/rcp/rsq/flr/ex2/lg2/litp, dphi/dsti/sgei/slti, and mad/madi.
...
def pica24_to_float(value: int) -> float:
    # Converts a 24-bit PICA float field stored in a u32 to a Python float.
    value &= 0xFFFFFF
    if value == 0:
        return 0.0
        ...
        for c in dvle.constants:
            ...
            for inp in dvle.inputs:
                ...
                if lo <= reg <= hi:
                    ...
                    if inp.name:
                        # If an input covers an array/range of registers, keep array indexing visible.
                        if hi > lo:
